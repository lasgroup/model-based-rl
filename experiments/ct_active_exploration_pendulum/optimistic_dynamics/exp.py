...
def experiment(
        seed: int = 0,
        project_name: str = 'CT_Optimistic_Active_Exploration',
    ...
    from mbrl.model_based_agent.continuous_active_exploration_model_based_agents import ContinuousOptimisticActiveExplorationModelBasedAgent
    ...
    swing_up_env = ContinuousPendulumEnv(reward_source='dm-control')
    swing_down_params = swing_up_env.reward_params.replace(target_angle=jnp.pi)
    swing_down_env = ContinuousPendulumEnv(reward_source='dm-control')
    swing_down_env.reward_params = swing_down_params

    class PendulumReward(Reward):
        def __init__(self, env: ContinuousPendulumEnv):
    ...
    agent = ContinuousOptimisticActiveExplorationModelBasedAgent(
        env=swing_up_env,
