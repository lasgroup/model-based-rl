...
# It adds the possibility to work with systems that do not measure state derivative
# It uses a smoother to estimate the state derivative
# The smoother is trained on the fly using the collected data
...
class Smoother_Wrapper(BaseAgentWrapper):
    ...
    def _split_transitions(transition: Transition,
                          indices: list[int],
                          ) -> list[Transition]:
