...
class RewardModel(object):
    ...
    def predict(self, obs: chex.Array, action: chex.Array,
                next_obs: Optional[chex.Array] = None) -> chex.Array:
        pass
...
class BicycleCarReward(RewardModel):
    def __init__(self, goal: chex.Array = jnp.zeros(3), action_cost: float = 0.0,
    ...
    def _init_fn(self):
        def predict(obs, action, next_obs=None, *args, **kwargs):
    ...
    def _predict(self, obs, action, next_obs=None):
...
class BicycleCarModel(DynamicsModel):
    ...
    def __init__(self, dt, reward_model: BicycleCarReward = BicycleCarReward()):
        ...
        self.predict = predict#jax.jit(predict)
        self.obs_dim = 6
    ...
    def reduce_x(self, x):
        ...
        x_reduced = jnp.concatenate([x[0:self.angle_idx], theta.reshape(-1),
    ...
    def _predict(self, x: jax.Array, u: jax.Array, params: CarParams):
        next_state = self.next_step(x, u, params)
        return next_state
